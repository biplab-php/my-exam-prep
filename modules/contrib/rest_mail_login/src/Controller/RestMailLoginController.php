 ...
 */
class RestMailLoginController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function login(Request $request) {
    ...
    $this->floodControl($request, $user->name->value);

    if ($this->userIsBlocked($user->name->value)) {
      throw new BadRequestHttpException('The user has not been activated or is blocked.');
    }
    
    if ($uid = $this->userAuth->authenticate($user->name->value, $credentials['pass'])) {
      $this->flood->clear('user.http_login', $this->getLoginFloodIdentifier($request, $user->name->value));
    ...
    if ($identifier = $this->getLoginFloodIdentifier($request, $credentials['mail'])) {
